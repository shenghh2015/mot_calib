...
class BoxDataLoader():
    ...
    def __init__(self, track_file         = '.',
        ...
        self.vel_dif_thresh     = vel_dif_thresh
        self.fps                = fps
    ...
    def load_tracks(self):
        ...
        # smoothen tracks
        smoothed_tracks = smoothen_tracks(track_boxes, use_clip = self.use_clip)
    ...
    def gen_box_pairs(self):
        ...
        for i in range(num_batchs):
            ...
            mid_frame = int(i * self.stride + self.window * clip_ratio / 2)
            ...
            box_pairs = masked_track_boxes[:, [0, mid_frame, -1], :]
            ...
            box_pairs = select_pairs_by_vel_dif(box_pairs, v_dif_thres = self.vel_dif_thresh)
...
# box_pairs: N x 3 x 4
# new_box_pairs: M x 3 x 4
def get_box_pts(boxes, loc):
    if loc == 'center':
...
# box_pairs: N x 3 x 4
# new_box_pairs: M x 3 x 4
def select_pairs_by_vel_dif(box_pairs, v_dif_thres, loc = 'center'):
    box_pts = get_box_pts(box_pairs, loc)
    # v1   = np.linalg.norm(box_pts[:, 1, :] - box_pts[:, 0, :], axis = -1)
    # v2   = np.linalg.norm(box_pts[:, 3, :] - box_pts[:, 2, :], axis = -1)
    v1   = np.linalg.norm(box_pts[:, 1, :] - box_pts[:, 0, :], axis = -1)
    v2   = np.linalg.norm(box_pts[:, 2, :] - box_pts[:, 1, :], axis = -1)
    inds = np.where(np.abs(v1 - v2) > v_dif_thres)[0]
    return box_pairs[inds, :, :]

...
from datasets import BoxDataLoader
from utils import load_seq_info, analyze_s_loss, analyze_alpha_loss, analyze_beta_loss, fov_constrain_loss
...
def main():
    ...
    # alpha, beta, fov = 59.66,1.45,55.1
    ...
    camera   = Camera(w_img = w_img, 
                    ...
                    cy = h_img / 2)
    aview        = AerialView(camera, rotation)
    constrain_loss1 = fov_constrain_loss(aview, rotation, boxes)
    print(f'alpha {alpha}, beta {beta}, fov {fov}, constrain loss: {np.round(constrain_loss1, 3)}')
    ...
    camera   = Camera(w_img = w_img, 
                    ...
                    cy = h_img / 2)
    aview        = AerialView(camera, rotation)
    constrain_loss2 = fov_constrain_loss(aview, rotation, boxes)
    print(f'alpha {alpha}, beta {beta}, fov {fov}, constrain loss: {np.round(constrain_loss2, 3)}')

...
from calib_losses import alpha_beta_fov_constrain_loss, alpha_beta_s_loss, alpha_beta_loss, s_loss, alpha_beta_fov_loss
import numpy as np
import os
import cv2
from scipy import optimize

from visualize import create_black_board, draw_text, vis_xyz_as_images, vis_xyz_as_images_v2, plot_loss_1d, vis_box_pairs
# from datasets import BoxDataLoader
from datasets3 import BoxDataLoader
from utils import analyze_fov_loss, load_seq_info, fov_constrain_loss, analyze_s_loss_v2, analyze_fov_constrain_loss
...
def dec_calib(boxes, bounds, cam_params, use_core = False, disp = True):
    ...
    params  = (cam_params, new_boxes)
    result  = optimize.differential_evolution(alpha_beta_fov_constrain_loss, [alpha, beta, fov], args = params, 
                    updating='deferred', tol = 1e-10, workers=4, disp = False, popsize=50)
...
def get_xy_ranges(cam_info, tracks):
...
def main():
    ...
    boxloader    = BoxDataLoader(track_file = track_file,
                                 ...
                                 height_dif_thresh  = 3,
                                 front_ratio_thresh = front_ratio_thresh,
                                 fps         = seq_info['fps'],
                                 use_clip    = use_clip,
                                 use_front   = use_front)
    ...
    # boxloader    = BoxDataLoader(track_file = track_file,
    #                              img_size    = (w_img, h_img),
    #                              window      = int(seq_info['fps'] * 4),
    #                              stride      = seq_info['fps'] // 5,
    #                              height_dif_thresh  = 3,
    #                              front_ratio_thresh = front_ratio_thresh,
    #                              fps         = seq_info['fps'],
    #                              use_clip    = use_clip,
    ...
    elif calib_method == 'dec':
        initials = ((20, 100), (-10, 10), (10, 200))
        result, loss_val = dec_calib(boxes, initials, cam_params, use_core = use_core, disp = False)
    ...
    print(f'alpha: {alpha}, beta: {beta}, fov: {fov}(s: {fov_to_s(fov)}) loss:{round(loss_val, 6)} dur: {round(duration, 2)} s')
    ...
    video_path = track_dir + f'/{seq_name}_{round(alpha, 2)}_{round(beta, 2)}_{round(fov, 2)}_{round(loss_val, 3)}_{calib_method}_{use_core}.mp4'
    print(video_path)
    fps        = 15
    ...
    rngs = get_xy_ranges(cam_info, all_tracks)

    vis_3d_draw = vis_xyz_as_images_v2(xyh_copy, track_ids, rngs, img_size = img_size, dot_size = 5, 
    ...
    output_vid.write(concat2)

    # cv2.imwrite(res_dir + f'/image_pts_{alpha}_{beta}_{fov}_v3.png', draw_img2)
    cv2.imwrite(res_dir + f'/image_pts_{seq_name}_{round(alpha, 2)}_{round(beta, 2)}_{round(fov, 2)}_{use_core}_{calib_method}.png', concat2)

    # check constrain loss
    constrain_loss = fov_constrain_loss(aview, rotation, boxes)
    print(f'alpha {alpha}, beta {beta}, fov {fov}, constrain loss: {np.round(constrain_loss, 3)}')

    # loss analysis
    boxes       = boxloader.get_all_pairs() # N x 2 x 4
    # s_range = [fov_to_s(120), fov_to_s(30)]
    # alpha, beta = 90.23, -2.03
    fov_range = [10, 200]
    params = (cam_params, boxes)
    losses, sols = analyze_fov_constrain_loss(alpha, beta, fov_range, 100, params)
    calib_res_dir = track_dir + '/calib2'
    gen_dir(calib_res_dir)
    plot_file = calib_res_dir + f'/a{round(alpha, 2)}_b{round(beta, 2)}_s-{fov_range}_loss_clip_{use_clip}.png'
    plot_loss_1d(plot_file, losses, sols, 
                title = 'fov loss', labels = ['fov', 'height loss'],
                use_clip = True)

    # draw the distribution of pairs
    analyze_pairs = True
    if analyze_pairs:
        track_dir = os.path.join(root_dir, 'results', proj_name, dataset_name, seq_name)
        vis_board = vis_box_pairs(w_img, h_img, boxes)
        dif_thresh = 3
        h_dif = np.abs(boxes[:, 0, 3] - boxes[:, 1, 3]) <= dif_thresh
        print(f'Number of pairs with dif not larger than {dif_thresh}: {np.sum(h_dif)}')
        cv2.imwrite(track_dir + f'/box_distributioin_{use_clip}.png', vis_board)

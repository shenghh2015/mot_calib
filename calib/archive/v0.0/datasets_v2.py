...
class BoxDataLoader():
    ...
    def gen_box_pairs(self):
        ...
        for i in range(num_batchs):
            ...
            ind_mask = get_ind_mask_v2(track_boxes[:,:,-1], front_mask, self.ind_mask)
...
# box points in a window
# def gen_pairs_v2(box_pts, front_mask, loc, thres, 
#                  fps = 20, front_ratio_thres = 0.8):
...
#     print(f'pair shape: {pair1.shape}, {pair2.shape}')
#     cond1 = trajectory_check(pair1, loc, thres)
#     cond2 = trajectory_check(pair2, loc, thres)
#     cond  = np.logical_and(cond1, cond2).flatten()

#     # compute front mask ratio
#     tracklet_len =  box_pts.shape[1]
#     front_ratio  =  front_mask.sum(axis = -1) / tracklet_len
#     print(f'front ratio: {front_ratio}')
#     print(f'cond1: {cond1.shape}, cond2: {cond2.shape}')

#     inds = np.where(np.logical_and(cond, front_ratio > front_ratio_thres))[0]
#     used_pair1 = pair1[inds, :, :]
#     used_pair2 = pair2[inds, :, :]
...
# check if a track is valid in a time window
def get_ind_mask_v2(mask, front_mask, ori_ind_mask, front_ratio_thresh = 0.8):
    ind_mask = ori_ind_mask.copy() == 1
    for i in range(mask.shape[-1]):
        ind_mask = np.logical_and(ind_mask, mask[:, i])
    
    box_mask    =  mask > 0
    front_ratio = front_mask.sum(axis = -1) / (box_mask.sum(axis = -1) + EPS)
    # print(front_ratio)
    ind_mask = np.logical_and(front_ratio > front_ratio_thresh, ind_mask)
    return ind_mask

...
from calib.geometry import compute_xyh_from_boxes, gen_focal_length, gen_focal_length_from_s, compute_fov
from calib.geometry_3d import Rotation
from calib_losses import alpha_beta_fov_loss_v4
import numpy as np
import os
import cv2
...
from utils import analyze_s_loss_v2, load_seq_info, analyze_alpha_loss_v2, analyze_beta_loss_v2
...
def main():
    ...
    result     = optimize.minimize(alpha_beta_fov_loss_v4, initials, args = params, 
                    method = 'BFGS', options={'gtol': 1e-10, 'disp': True})
    ...
    focal_length   = w_img /2  * s
    fov  = compute_fov(w_img, focal_length)
    ...
    s_range = [0.75, 3.7]
    s_losses, s_space = analyze_s_loss_v2(alpha, beta, s_range, 
                           100, params)
    plot_loss_1d(calib_res_dir + '/fov_loss.png', s_losses, s_space, 
                title = 'fov loss', labels = ['fov', 'height loss'],
                use_clip = True)

    alpha_range = [0, 95]
    alpha_losses, alpha_space = analyze_alpha_loss_v2(alpha_range, beta, s, 
    ...
    beta_range = [-10, 10]
    beta_losses, beta_space = analyze_beta_loss_v2(alpha, beta_range, s, 
    ...
    focal_length = gen_focal_length_from_s(w_img, s)

...
class BoxDataLoader():
    ...
    def __init__(self, track_file         = '.',
        ...
        # smoothen tracks
        self.tracks = smoothen_tracks(self.tracks, use_clip = self.use_clip)
        ...
        # self.ecc_coefs          = self.gen_ecc_coefficients(fps = self.fps, time_window = time_window)
    ...
    #         if frame_id == frame_id + frame_window // 2: continue
    ...
    def gen_ecc_coefficients_v2(self, fps = 30, time_window = 3):
        ...
        for frame_id in range(num_frames):
            ...
            if frame_id % frame_window == 0:
                ref_frame     = cv.imr ead(self.img_dir + '/{:06d}.jpg'.format(ref_frame_id + 1))
            frame = cv.imread(self.img_dir + '/{:06d}.jpg'.format(frame_id + 1))

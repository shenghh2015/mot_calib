from geometry import gen_focal_length, compute_xyh_from_boxes
...
from const import MOT17_TRAIN_SEQ_NAMES, RANDOM_COLORS
...
def main():
    ...
    # test new aview view object
    alpha, beta, fov = 68.32, 0.13, 33.58
    focal_length     = gen_focal_length(w_img, fov)
    camera.update_intrisic(np.array([focal_length, focal_length, w_img / 2, h_img/ 2]))
    ...
    print(f'alpha: {round(alpha, 2)}, beta: {round(beta, 2)}, fov: {round(fov, 2)}')

    # visualize calibration result
    track_dir     = os.path.join(root_dir, 'results', proj_name, dataset_name, seq_name)
    res_dir       = track_dir + '/ground_floor2'
    gen_dir(res_dir)
    ...
    frame        = cv2.imread(img_dir + '/{:06d}.jpg'.format(frame_id + 1))
    ...
    for i in range(img_pts.shape[0]):
        ...
        # cv2.line(draw_img, top_pt, bot_pt, (0, 255, 0), thickness)
    ...
    # rays    = rotation.rotate_inverse(xyhs)
    # img_pts = camera.map_rays3d_to_pts2d(rays)
    ...
    cv2.imwrite(res_dir + f'/image_{seq_name}_{round(alpha, 2)}_{round(beta, 2)}_{round(fov, 2)}.png', concat)

...
def main():
    ...
    for frame_id in range(ref_frame_id, ref_frame_id + num_frames):
        ...
        rgb_dif = np.stack([g_diff2, g_diff2, g_diff2], axis = -1)
        ...
        # fig.tight_layout()
        # plt.plot(cdx_list, color='b')
        # plt.plot(cdy_list, color='g')
        # plt.plot(cdz_list, color='r')
        # plt.legend(['accum_x', 'accum_y', 'accum_z'])

        # plt.xlim([0, num_frames])
        # # plt.ylim([0, 90])
        # plt.close(fig)

        # img3 = convert_pyplot_to_image(fig, dpi=120)
        ...
        # draw_concat2 = cv2.vconcat([draw_concat2, img3])

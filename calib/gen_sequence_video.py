...
def get_params():
    ...
    parser.add_argument("--use_full",     action="store_true", help="use ground truth or not")
...
def main():
    ...
    # save_to_json(calib_res_dir + f'/{seq_name}.json', cam_info)
    ...
    frame_id = 150
    num_frames = 100
    num_frames = min(num_frames, all_tracks.shape[1])
    ...
    # seq_vis_dir = os.path.join(root_dir, 'results', dataset_name, subset, seq_name)

    # if args.use_full: seq_vis_dir += '_full'

    seq_vis_dir = os.path.join(root_dir, 'results', dataset_name, subset)

    gen_dir(seq_vis_dir)
    video_path = seq_vis_dir + f'/{seq_name}.mp4'
    print(video_path)
    fps        = seq_info['fps']
    output_vid = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (w_img, h_img))
    ...
    for frame_id in range(num_frames):
        ...
        #     bot_pt = img_pts[i, 1, :]
        #     track_id = track_ids[i]
        #     cv2.line(draw_img, top_pt, bot_pt, RANDOM_COLORS[track_id].tolist(), thickness)

        # concat = cv2.hconcat([draw_img, blk_board])
        output_vid.write(draw_img)

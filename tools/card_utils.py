# fix the bugs related to transition
import .geometry_utils as gm3d
import numpy as np
...
class CardBoard:
    ...
    def bboxes2quadrangles3d(self, bboxes):
        ...
        # scale with camera height
        quadrangle_cboards = quadrangle_cboards
    ...
    def bboxes2cboards(self, bboxes, use_world = False):
        bboxes = bboxes.reshape((-1, 4))
        quad3d, base_u, base_v  = self.bboxes2quadrangles3d(bboxes)
        cboards = cut_quad3d(quad3d, base_u, base_v)

        # add camera motion to get world coordinates
        if use_world:
            cboards = cboards * self.cam_height + self.cam_pos.reshape((1, 1, 3))

        return cboards

    def cboards2quadrangles2d(self, cardboards, use_world = False):
        # translate the cardboard coordinates back to the camera coordinate system
        pts_3d       = cardboards.reshape((-1, 3))
        
        if use_world:
            pts_3d  = pts_3d - self.cam_pos.reshape((1, 3))
    ...
    def cboards2bboxes(self, cboards, use_world = False):
        cboards = cboards.reshape((-1, 4, 3))
        quad2d  = self.cboards2quadrangles2d(cboards, use_world = use_world)
        return get_max_2d_tlwhs(quad2d)

    # map from image to ground floor
    def map_img_pts_to_grf_pts(self, box_pts, use_world = False):
        ...
        # add camera motion
        if use_world:
            xyz = xyz * self.cam_height

        return xyz[:, :2]

    # map from ground floor
    def map_grf_pts_to_img_pts(self, xy, use_world = False):
        xy      = xy.reshape((-1, 2))
        pts3d   = np.concatenate([xy, np.ones((xy.shape[0], 1))], axis = -1)

        if use_world:
            pts3d   = pts3d - self.cam_pos.reshape((1, 3))
    ...
    # def set_transition(self, transition = np.array([0, 0, 0])):
    #     self.transition = transition.copy()

    def set_transition(self, cam_pos = np.array([0, 0, 0])):
